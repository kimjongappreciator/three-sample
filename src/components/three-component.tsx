...
function createImpact(x: number, y: number, strength = 1, radius = 1) {
  ...
  impacts = impacts.filter(
    (impact) => performance.now() - impact.time < 5000 // 5 segundos
  );
}
...
function ThreeComponent() {
  ...
  useEffect(() => {
    ...
    updateSize(); // Llamar inicialmente
    ...
    reflectionGeometry.setAttribute(
      "position",
      new Three.BufferAttribute(reflectionPositions, 3)
    );
    reflectionGeometry.setAttribute(
      "color",
      new Three.BufferAttribute(reflectionColors, 3)
    );
    ...
    const reflectionMesh = new Three.Points(
      reflectionGeometry,
      reflectionMaterial
    );
    ...
    function animateWater(time: number) {
      ...
      const reflectionPositionsArray =
        reflectionGeometry.attributes.position.array;
      ...
      for (let i = 0; i < reflectionCount; i++) {
        ...
        const waveEffect1 =
          Math.sin(t * 3 + x * 0.1) * Math.sin(t * 2.5 + z * 0.1);
        ...
        if (finalIntensity > 0.1) {
          ...
          reflectionPositionsArray[i3 + 1] =
            -1.35 + Math.sin(t * 4 + i * 0.1) * 0.03;
        } else {
